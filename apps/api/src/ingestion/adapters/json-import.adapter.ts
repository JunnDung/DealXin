...
export class JsonImportAdapter implements SourceAdapter {
  ...
  async parse(
    ...
    for (const raw of rawDeals) {
      ...
      const srcUrl = (deal['sourceUrl'] as string) || (deal['url'] as string) || (deal['link'] as string);
      if (srcUrl) item.sourceUrl = srcUrl;
      const imgUrl = (deal['imageUrl'] as string) || (deal['image_url'] as string) || (deal['image'] as string);
